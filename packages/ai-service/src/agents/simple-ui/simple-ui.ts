...
export function printDetailedGameState(state: any): void {
  ...
  console.log(
    `${COLORS.bright}Dungeon:${COLORS.reset} ${state.currentDungeon}`,
  );
  console.log(`${COLORS.bright}Room:${COLORS.reset} ${state.currentRoom}`);
  console.log(
    `${COLORS.bright}Loot Phase:${COLORS.reset} ${state.lootPhase === 'true' ? 'Yes' : 'No'}`,
  );
  ...
  if (state.lastBattleResult) {
    ...
    console.log(
      `${COLORS.bright}Result:${COLORS.reset} ${state.lastBattleResult}`,
    );
    console.log(
      `${COLORS.bright}Enemy Move:${COLORS.reset} ${state.lastEnemyMove}`,
    );
  }
  ...
  const healthPercentage =
    Math.floor(
      (parseInt(state.playerHealth) / parseInt(state.playerMaxHealth)) * 100,
    ) || 0;
  const healthBar = createProgressBar(healthPercentage, COLORS.red);
  console.log(
    `${COLORS.bright}Health:${COLORS.reset} ${state.playerHealth}/${state.playerMaxHealth} ${healthBar}`,
  );

  const shieldPercentage =
    Math.floor(
      (parseInt(state.playerShield) / parseInt(state.playerMaxShield)) * 100,
    ) || 0;
  const shieldBar = createProgressBar(shieldPercentage, COLORS.blue);
  console.log(
    `${COLORS.bright}Shield:${COLORS.reset} ${state.playerShield}/${state.playerMaxShield} ${shieldBar}`,
  );

  // Weapon stats
  console.log(`\n${COLORS.bright}Weapon Stats:${COLORS.reset}`);
  console.log(
    `${COLORS.red}Rock:${COLORS.reset} ATK ${state.rockAttack} | DEF ${state.rockDefense} | Charges ${state.rockCharges}`,
  );
  console.log(
    `${COLORS.green}Paper:${COLORS.reset} ATK ${state.paperAttack} | DEF ${state.paperDefense} | Charges ${state.paperCharges}`,
  );
  console.log(
    `${COLORS.blue}Scissor:${COLORS.reset} ATK ${state.scissorAttack} | DEF ${state.scissorDefense} | Charges ${state.scissorCharges}`,
  );
  ...
  if (parseInt(state.enemyHealth) > 0 || state.currentEnemy !== '0') {
    ...
    console.log(
      `${COLORS.bright}Enemy ID:${COLORS.reset} ${state.currentEnemy}`,
    );

    const enemyHealthPercentage =
      Math.floor(
        (parseInt(state.enemyHealth) / parseInt(state.enemyMaxHealth)) * 100,
      ) || 0;
    const enemyHealthBar = createProgressBar(enemyHealthPercentage, COLORS.red);
    console.log(
      `${COLORS.bright}Health:${COLORS.reset} ${state.enemyHealth}/${state.enemyMaxHealth} ${enemyHealthBar}`,
    );

    const enemyShieldPercentage =
      Math.floor(
        (parseInt(state.enemyShield) / parseInt(state.enemyMaxShield)) * 100,
      ) || 0;
    const enemyShieldBar = createProgressBar(
      enemyShieldPercentage,
      COLORS.blue,
    );
    console.log(
      `${COLORS.bright}Shield:${COLORS.reset} ${state.enemyShield}/${state.enemyMaxShield} ${enemyShieldBar}`,
    );
  }
  ...
  if (
    ...
    state.lootOptions.forEach((loot: any, index: number) => {
      console.log(
        `${COLORS.bright}Option ${index + 1}:${COLORS.reset} ${loot.boonTypeString} (Rarity: ${loot.RARITY_CID})`,
      );
    });
  }
