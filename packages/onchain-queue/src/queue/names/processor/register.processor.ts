...
@Processor(ONCHAIN_QUEUES.QUEUE_REGISTER_NAME)
export class RegisterProcessor {
  ...
  logger = new Logger(RegisterProcessor.name);
  @Process({ name: ONCHAIN_JOBS.JOB_REGISTER_NAME, concurrency: 100 })
  async processRegisterName(job: Job<LogsReturnValues>) {
    ...
    try {
      ...
      this.logger.error(
        `Failed to detect tx hash - reigster ${event.txReceipt.hash}`,
      );
      this.queue.add(ONCHAIN_JOBS.JOB_REGISTER_NAME, event);
    }
  }
}
